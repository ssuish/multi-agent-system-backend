...
- **Conversations:** `POST /conversations`, `GET /conversations`, `GET /conversations/{conversation_id}`, `DELETE /conversations/{conversation_id}`, and `GET /conversations/{conversation_id}/messages` in [`app/api/v1/conversations.py`](../app/api/v1/conversations.py), backed by [`app/services/conversation_service.py`](../app/services/conversation_service.py).
- **Chat:** `POST` `/conversations/{conversation_id}/messages` in [`app/api/v1/chat.py`](../app/api/v1/chat.py), backed by [`app/services/chat_service.py`](../app/services/chat_service.py) and the shared ADK `Runner` from [`app/agent_runtime.py`](../app/agent_runtime.py). The route returns a 404 when the conversation does not exist for the caller.
